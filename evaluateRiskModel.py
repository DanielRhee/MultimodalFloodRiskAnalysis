...
class RiskEvaluator:
    ...
    @torch.no_grad()
    def evaluate(self, image, depthMap=None, tile_size=128, stride=128):
        w, h = image.size
        rgb_full = np.array(image).astype(np.float32)
        rm_rgb_norm = (rgb_full - np.array(rm_config.RGB_MEAN)) / np.array(rm_config.RGB_STD)

        if depthMap is not None:
            elev_full = np.array(depthMap).astype(np.float32)
            if len(elev_full.shape) == 2:
                elev_full = elev_full[:, :, np.newaxis]
        else:
            elev_full = np.zeros((h, w, 1), dtype=np.float32)
        ...
        for y in range(0, h - tile_size + 1, stride):
            for x in range(0, w - tile_size + 1, stride):
                ...
                elev_tile = elev_full[y:y+tile_size, x:x+tile_size]

                if depthMap is not None:
                    tileMin = elev_tile.min()
                    tileMax = elev_tile.max()
                    if tileMax > tileMin:
                        elev_tile = (elev_tile - tileMin) / (tileMax - tileMin)
                    else:
    ...
    @torch.no_grad()
    def getLandClassification(self, image):
        rgb_array = np.array(image).astype(np.float32)
        h, w = rgb_array.shape[:2]

        if len(rgb_array.shape) == 3 and rgb_array.shape[2] == 3:
            nir = np.zeros((h, w, 1), dtype=np.float32)
            rgbi = np.concatenate([rgb_array, nir], axis=2)
        else:
            rgbi = rgb_array

        normalized = (rgbi - np.array(lc_config.NORMALIZATION_MEAN)) / np.array(lc_config.NORMALIZATION_STD)
        tensor = torch.from_numpy(normalized.transpose(2, 0, 1)).unsqueeze(0).float().to(self.device)

        output = self.lc_model(tensor)
        pred = torch.argmax(output, dim=1).cpu().numpy()[0]

        return pred

    def visualizeRiskToBytes(self, image, risk_mask, tile_size=128):
